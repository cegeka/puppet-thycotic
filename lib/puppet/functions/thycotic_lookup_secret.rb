Puppet::Functions.create_function(:thycotic_lookup_secret) do
  ...
  # Load the Thycotic object using the correct config
  def init()
    [
    ...
    ].each do |path| # Try to load the config from a couple of paths
      begin
        @cfg_file = ParseConfig.new(path)
        break
      rescue Exception => e
    ...
    if @cfg_file.nil?
      raise Puppet::ParseError, 'Could not load configuration. Please see "README". Supplied config paths: #{p}.'
    end
